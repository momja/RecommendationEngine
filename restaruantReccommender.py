...
def cosineDistance(main,
                   user):  # takes two items and calculates the cosine distance between them and returns it to the compare function
    dotProduct = sum([main[i] * user[i] for i in range(len(user))])
    length = math.sqrt(sum([main[i] ** 2 for i in range(len(main))])) * math.sqrt(
        sum([user[i] ** 2 for i in range(len(user))]))
...
def compare(main, users):  # calls the cosine distance function and returns the user most similar to user A
    ...
    for user in users:
        ...
        topSimilarities.append(
            (distance, user))  # holds the cosine distance between each user and the main, and the user's ratings

    def getKey(item):
        return item[0]

    topSimilarities.sort(key=getKey, reverse=True)
    relevantUsers = [x[1] for x in
                     topSimilarities]  # ignores the cosine distance when going into the recommend function
